...
function initMap() {
  ...
  var myLatlng = {lat: Number(document.getElementById("la").value), lng: Number(document.getElementById("lo").value)};
  ...
  document.getElementById("usr").value =  document.getElementById("la").value + ", " +   document.getElementById("lo").value;  
  addMarker(myLatlng);
...
function showNetworks() {
  $("#accordion").html("");
  var wifiPar = document.getElementById("ws").value;
  var wifiString = wifiPar.replace(/\(/g, "\"");
  console.log(wifiString);
  var wifi = JSON.parse(wifiString);
  console.log(wifi);
  for (var i = 0; i < wifi.length; i++) {
	console.log(i);
    //Obtenemos la informacion de cada red wifi
	//IMPORTANTE LAS MAYUSCULAS Y ESOOO
